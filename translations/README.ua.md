...
CodePulse — це не просто ще один лінтер; це перший у галузі **двигун архітектурної розвідки**. У той час як звичайні інструменти (ESLint, SonarQube, CodeClimate) зосереджуються на симптоматичних, ізольованих перевірках на рівні файлів, CodePulse надає цілісний архітектурний огляд, об'єднуючи п'ять критичних вимірів даних:

*   **Структурний аналіз AST:** Глибоке семантичне розуміння модулів коду.
*   **Git-темпоральна аналітика:** Відстежує «приховані» зв'язки через історію змін (churn та co-change rates).
*   **Історична Time Machine:** Візуалізує архітектурну деградацію протягом історії комітів.
*   **Кореляція з Runtime:** Накладає вузькі місця продуктивності на статичні структури коду.
*   **Нативна розвідка MCP:** Надає LLM структурний контекст, необхідний для точного, агентного рефакторингу.

**«Відсутня ланка»:** В епоху AI-інженерії якість роботи ШІ обмежена контекстом, який він отримує. CodePulse слугує основним **постачальником архітектурного контексту**. Ми не просто звітуємо про технічний борг — ми проактивно ідентифікуємо найбільш впливові шляхи рефакторингу, надаючи точну структурну карту, необхідну LLM для виправлення архітектурних проблем із хірургічною точністю.
