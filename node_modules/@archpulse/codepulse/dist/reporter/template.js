...
const template_parts_1 = require("./template-parts");
...
function renderComplexityRows(stats) {
    return stats.top10Complex.map((f) => {
...
function renderVulnerabilityRows(stats) {
    return stats.vulnerabilitiesList.map((v) => `<tr>
...
function renderDeadRows(result) {
    return result.deadExports.slice(0, 50).map(d => `<tr><td class="td-path">${d.file}</td><td><code>${d.name}</code></td></tr>`).join('\n');
}
function renderGodRows(stats) {
    return stats.godFilesList.map((f) => `<tr><td class="td-path">${f.path}</td><td>${f.lines}</td><td>${f.imports}</td><td>${f.complexity}</td></tr>`).join('\n');
}
function renderCriticalRows(result) {
...
function renderHotspotRows(stats) {
    return stats.hotspots.map((h) => `
    <tr>
...
function buildHtml(result, stats) {
...
<div class="container">
  ...
  <div class="grid-4" style="margin-top:-24px">
    ...
    <div class="stat-card">
      ...
      <div class="value" style="color:var(--green)">${(0, html_1.calculateHealthScore)(stats, result)}<span style="font-size:20px;color:var(--muted)">/100</span></div>
...
${(0, template_parts_1.getScripts)(stats.treemapData, stats.graphData)}
</body>
</html>`;
}
