...
function printStats(result, dir) {
    ...
    console.log('  ' + chalk_1.default.gray(SYMBOLS.thinLine.repeat(30)));
    if (result.deadExports.length > 0) {
        console.log(`  ${chalk_1.default.red(SYMBOLS.cross)} Dead exports:    ${chalk_1.default.red(result.deadExports.length)}`);
    }
    else {
        console.log(`  ${chalk_1.default.green(SYMBOLS.check)} Dead exports:    ${chalk_1.default.green('none')}`);
    }
    if (result.godFiles.length > 0) {
        console.log(`  ${chalk_1.default.yellow(SYMBOLS.warn)} God files:       ${chalk_1.default.yellow(result.godFiles.length)}`);
    }
    else {
        console.log(`  ${chalk_1.default.green(SYMBOLS.check)} God files:       ${chalk_1.default.green('none')}`);
    }
    if (result.criticalFiles.length > 0) {
        ...
        console.log(`  ${chalk_1.default.green(SYMBOLS.check)} Critical nodes:  ${chalk_1.default.green('none')}`);
    }
    if (result.hotspots.length > 0) {
        console.log(`  ${chalk_1.default.magenta(SYMBOLS.fire)} Hotspots:        ${chalk_1.default.magenta(result.hotspots.length)}`);
    }
    else {
        console.log(`  ${chalk_1.default.green(SYMBOLS.check)} Hotspots:        ${chalk_1.default.green('none')}`);
    }
    const vulnerabilities = result.issues.filter(i => i.type === 'vulnerability');
    if (vulnerabilities.length > 0) {
        console.log(`  ${chalk_1.default.red(SYMBOLS.radio)} Vulnerabilities: ${chalk_1.default.red.bold(vulnerabilities.length)}`);
    }
    else {
        console.log(`  ${chalk_1.default.green(SYMBOLS.check)} Vulnerabilities: ${chalk_1.default.green('none')}`);
    }
    console.log('\n' + chalk_1.default.cyan(SYMBOLS.line.repeat(52)) + '\n');
}
function printDeadCode(result) {
    console.log('\n' + chalk_1.default.bold.red('Dead Code — Unused Exports'));
    console.log(chalk_1.default.gray(SYMBOLS.thinLine.repeat(50)));
    if (result.deadExports.length === 0) {
        console.log(chalk_1.default.green(`  ${SYMBOLS.check} No dead exports found!`));
        return;
    }
...
function printGodFiles(result) {
    ...
    console.log(chalk_1.default.gray(SYMBOLS.thinLine.repeat(50)));
    if (result.godFiles.length === 0) {
        console.log(chalk_1.default.green(`  ${SYMBOLS.check} No god files found!`));
        return;
    }
