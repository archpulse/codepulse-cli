...
function buildHtml(result, stats) {
    ...
    const deadRows = result.deadExports.slice(0, 50).map(d => `<tr><td class="td-path">${d.file}</td><td><code>${d.name}</code></td></tr>`).join('\n');
    const godRows = stats.godFilesList.map(f => `<tr><td class="td-path">${f.path}</td><td>${f.lines}</td><td>${f.imports}</td><td>${f.complexity}</td></tr>`).join('\n');
