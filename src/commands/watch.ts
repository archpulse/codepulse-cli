...
export async function runWatch(dir: string, opts: any = {}) {
	...
	if (process.stdin.isTTY) {
		...
		process.stdin.on("keypress", (_str, key) => {
			if (key.ctrl && key.name === "c") {
				...
				if (_lastResult) renderDashboard(_lastResult, isAnalyzing);
			}
		});
	}
	...
	const performAnalysis = async () => {
		...
		if (_lastResult) {
			renderDashboard(_lastResult, true);
		}
	...
	const renderIssuesList = (issues: any[], showFull: boolean) => {
	...
	const renderDashboard = (result: AnalysisResult, analyzing: boolean) => {
		const {
			totalFiles,
			totalLines,
			avgComplexity,
