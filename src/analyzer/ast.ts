...
function handleNamedExportSpecifiers(specifiers: any[], exports: string[]) {
  specifiers.forEach((spec: any) => {
    if (t.isExportSpecifier(spec)) {
      exports.push(t.isIdentifier(spec.exported) ? spec.exported.name : (spec.exported as any).value);
    }
  });
}

function handleNamedExport(p: NodePath<t.ExportNamedDeclaration>, exports: string[]) {
  const { node } = p;
  if (node.declaration) {
...
function handleDefaultExport(p: NodePath<t.ExportDefaultDeclaration>, exports: string[]) {
  const { node } = p;
...
function handleModuleExport(p: NodePath<t.AssignmentExpression>, exports: string[]) {
  const { node } = p;
  if (
...
function handleFunction(p: NodePath<any>, functions: FunctionNode[], exports: string[]) {
  const { node, parent } = p;
