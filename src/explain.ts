export const EXPLANATIONS = {
	...
	"dead-export": {
		...
		full: {
			description:
				"A Dead Export is a symbol exported from a file but never used by any other module.",
			criteria: [
	...
	"critical-node": {
		...
		full: {
			description:
				"A Critical Node is a file that acts as a hub — many other modules depend on it.",
			criteria: ["Imported by 10+ other files (configurable)"],
			risks: [
	...
	"high-complexity": {
		...
		full: {
			...
			criteria: ["Complexity > 10 per function"],
			risks: [
	...
	vulnerability: {
		...
		full: {
			description:
				"A vulnerability is a flaw that could be exploited by an attacker.",
			criteria: [
	...
	duplication: {
		...
		full: {
			description:
				"Code duplication occurs when the same or very similar logic exists in multiple places.",
			criteria: ["15+ lines of identical code (configurable)"],
			risks: [
	...
	"dependency-vulnerability": {
		...
		full: {
			description:
				"Software Composition Analysis (SCA) detected libraries with public CVEs.",
			criteria: ["Dependency version matches known vulnerable range"],
			risks: [
